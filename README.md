...
В репозитории находится файл `example-payload.json` с минимальным рабочим примером данных для тестирования эндпоинта `/calculate`.
...
# В другом терминале выполнить тестовый запрос
curl -X POST http://localhost:3100/calculate \
  -H "Content-Type: application/json" \
  -d @example-payload.json
```

### Структура example-payload.json

Файл содержит:
- `elementsStore` с минимальными данными CALC_DETAILS, CALC_STAGES, CALC_SETTINGS
- Одно торговое предложение в `selectedOffers`
- Базовую информацию о продукте в `product`
- Пресет с настройкой цен в `preset`
- Типы цен в `priceTypes`

Этот пример демонстрирует базовую работу калькулятора без сложной логики LOGIC_JSON.

## Отличия от фронтенд-версии

Серверная версия калькулятора имеет следующие отличия от фронтенд-версии (calcconfig):

1. **Без Zustand-сторов** — все данные передаются через `initPayload`
2. **Без fallback-логики** — если LOGIC_JSON не задан, стоимость будет 0
3. **Логирование через переменные окружения** — вместо `console.log` используется настраиваемый logger
4. **Отсутствие UI-зависимостей** — типы и функции не содержат ссылок на браузер или React

## Производительность

- Расчёт одного ТП: ~10-50 мс (зависит от сложности)
- Поддержка больших payload: до 50MB
- Асинхронная обработка с возможностью масштабирования через Docker
...
При этом в логи будут выводиться:
- Детали трансформации данных
- Контексты расчёта для каждого этапа
- Промежуточные результаты вычисления переменных
- Значения всех входов и выходов

## Безопасность и лучшие практики

1. **Всегда используйте ALLOWED_IPS в production** для ограничения доступа
2. **Настройте CORS_ORIGIN** если известен точный origin клиента
3. **Используйте HTTPS** в production (настройте reverse proxy типа nginx)
4. **Ограничьте размер payload** если нужно (по умолчанию 50MB)
5. **Мониторинг** — используйте `/health` для healthchecks

## Troubleshooting (продолжение)

### Проблема: Медленная работа

- Проверьте размер elementsStore — большие объёмы данных увеличивают время обработки
- Включите LOG_LEVEL=debug для анализа узких мест
- Рассмотрите оптимизацию LOGIC_JSON формул

### Проблема: Ошибки парсинга формул

- Убедитесь что LOGIC_JSON содержит корректный JSON
- Проверьте синтаксис формул в переменных
- Используйте только поддерживаемые функции: `get`, `ceil`, `floor`, `max`, `min`, `abs`, `trim`, `lower`, `round`, `if`, `upper`, `len`, `contains`, `replace`, `toNumber`, `toString`, `join`, `regexMatch`, `regexExtract`, `getPrice`, `split`
...
https://github.com/prospektweb/calc-server/issues

## История изменений

### v1.0.0 (2024-01-15)
- Первый релиз
- Адаптация движка калькуляции из calcconfig
- Поддержка LOGIC_JSON
- IP-фильтрация
- Docker-контейнер
