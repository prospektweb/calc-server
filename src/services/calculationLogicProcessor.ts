...
export function extractLogicDefinition(settingsElement: ElementsStoreItem | null | undefined): LogicDefinition | null {
  ...
  try {
    ...
    logger.warn('[CALC] Failed to parse LOGIC_JSON:', e)
    return null
  }
}
...
const extractOptionsMapping = (
  ...
  try {
    ...
    logger.warn('[CALC] Failed to parse options mapping JSON:', error)
    return null
  }
}
  ...
  for (const wiring of inputWirings) {
    if (wiring.sourcePath) {
      ...
      if (stageAliasMatch) {
        ...
        logger.debug('[CALC] Wired input:', wiring.paramName, '=', aliasValue, 'from', wiring.sourcePath)
        continue
      }
      ...
      logger.debug('[CALC] Wired input:', wiring.paramName, '=', value, 'from', wiring.sourcePath)
    }
  }

  logger.debug('[CALC] Built context with', Object.keys(context).length, 'keys')
  return context
}
...
export function evaluateFormula(formula: string, context: Record<string, any>): number | string | boolean | undefined {
  ...
  try {
    ...
    logger.warn('[CALC] Failed to evaluate formula:', formula, e)
    return undefined
  }
}
...
export function evaluateLogicVars(
  ...
  if (!logicDefinition?.vars || !Array.isArray(logicDefinition.vars)) {
    logger.debug('[CALC] No vars in logic definition')
    onLog?.({ type: 'noVars' })
    return {}
  }
  ...
  logger.debug('[CALC] Evaluating', logicDefinition.vars.length, 'variables')
  ...
  for (const varDef of logicDefinition.vars) {
    ...
    if (varDef.formula) {
      ...
      logger.debug('[CALC] Var', varDef.name, '=', value, 'from formula:', varDef.formula)
      onLog?.({
        type: 'varFormula',
        name: varDef.name,
        value,
      ...
      logger.debug('[CALC] Var', varDef.name, '=', varDef.value, '(static)')
      onLog?.({
        type: 'varStatic',
        name: varDef.name,
        value: varDef.value,
