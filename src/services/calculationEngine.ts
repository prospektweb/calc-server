 ...
 * 
 * Adapted from prospektweb/calcconfig for server-side calculation
...
// InitPayload interface (adapted from @/lib/postmessage-bridge)
export interface InitPayload {
  elementsStore: Record<string, any[]>
...
async function calculateStage(
  ...
  logger.debug('[CALC] ==> Processing stage:', {
    stageId: stage.id,
  ...
  if (settingsElement && stageElement && stage.stageId) {
    try {
      ...
      logger.debug('[CALC] Logic extraction:', {
        stageId: stage.id,
      ...
      if (logicDefinition) {
        ...
        logger.debug('[CALC] Calculation context built:', {
          stageId: stage.id,
        ...
        logger.debug('[CALC] Variables evaluated:', {
          stageId: stage.id,
        ...
        logger.debug('[CALC] Logic applied for stage:', stage.id, {
        ...
        if (operationPurchasing !== undefined || materialPurchasing !== undefined) {
          ...
          logger.warn('[CALC] Legacy OUTPUTS keys purchasingPrice/basePrice are ignored for added costs. Use operation*/material* mappings instead.', {
            stageId: stage.id,
            outputs: outputValues,
          })
        }
      } else {
        logger.debug('[CALC] No logic definition or outputs for stage:', stage.id)
      }
    } catch (error) {
      logger.warn('[CALC] Logic processing failed for stage:', stage.id, error)
    }
  }
  
  // Server-side: No fallback to Zustand stores
  // If logic is not applied or produced no output values, costs remain 0
...
async function calculateDetail(
  ...
  logger.debug('[CALC] ===> Processing detail:', {
    detailId: detail.id,
    detailName: detail.name,
  ...
  logger.debug('[CALC] Detail calculation complete:', {
    detailId: detail.id,
    detailName: detail.name,
  ...
  const result: CalculationDetailResult = {
    ...
    basePrice: detailBasePrice || (typeof derivedBasePrice === 'number' ? derivedBasePrice : totalCost),
    currency: 'RUB',
...
async function calculateBinding(
  ...
  logger.debug('[CALC] ===> Processing binding:', {
  ...
  if (visited.has(binding.id)) {
    logger.warn(`[CALC] Circular reference detected in binding: ${binding.id}`)
    return {
      detailId: binding.id,
...
export async function calculateOffer(
  ...
  logger.debug('[CALC] ====> Starting offer calculation:', {
    offerId: offer.id,
  ...
  logger.debug('[CALC] Top-level items identified:', {
  ...
  logger.debug('[CALC] Offer calculation complete:', {
    offerId: offer.id,
    offerName: offer.name,
