...
export function transformStage(stageElement: CalcStageElement): StageInstance {
  ...
  logger.debug('[transformStage] Transforming stage:', {
    id: stageElement.id,
...
export function transformDetail(
  ...
  logger.debug('[transformDetail] Transforming detail:', {
    id: detailElement.id,
  ...
  logger.debug('[transformDetail] Stage IDs to process:', {
    stageIds,
    stageIdArray,
  ...
  if (elementsStore.CALC_STAGES) {
    stageIdArray.forEach(stageId => {
      ...
      logger.debug('[transformDetail] Looking for stage:', {
        stageId,
        stageIdNum,
      })
      const stageElement = elementsStore.CALC_STAGES.find(s => s.id === stageIdNum)
      if (stageElement) {
        logger.debug('[transformDetail] Found stage element:', stageElement)
        stages.push(transformStage(stageElement as CalcStageElement))
      } else {
        logger.warn('[transformDetail] Stage not found in store:', stageIdNum)
      }
    })
  }
  ...
  logger.debug('[transformDetail] Result:', {
    id: result.id,
    name: result.name,
...
export function transformPresetToUI(
  ...
  logger.debug('[transformPresetToUI] Starting transformation:', {
    presetId: preset.id,
  ...
  topLevelIds.forEach(detailId => {
    ...
    logger.debug('[transformPresetToUI] Processing top-level ID:', { detailId, detailIdNum })
    const element = elementsStore.CALC_DETAILS.find(d => d.id === detailIdNum) as CalcDetailElement
    
    if (!element) {
      logger.warn('[transformPresetToUI] Element not found for ID:', detailIdNum)
      return
    }
    
    const elementType = element.properties.TYPE?.VALUE_XML_ID
    logger.debug('[transformPresetToUI] Found element:', { 
      id: element.id, 
      name: element.name, 
      type: elementType 
    })
  ...
  logger.debug('[transformPresetToUI] Processing complete:', {
  ...
  logger.debug('[transformPresetToUI] Final result:', {
